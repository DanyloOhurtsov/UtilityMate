...
function HEADER(){
    ...
    headerBtnMobile.addEventListener('click', function(){
        this.closest('header').querySelector('.headerMobile').querySelector('.menu').classList.toggle('none')
    })
    let menuBtn = document.querySelectorAll('.headerMobile .menu ul li');
    menuBtn.forEach(item=>{
        item.addEventListener('click', function(){
            this.closest('.menu').classList.add('none')
        })
    })
}
HEADER()
...
function ALL(){
    ...
    function CALCULATORFUNC(){
        ...
        function calculateItemFunc(){
            ...
            let idCal = perentItem.id;
            let perentRes = this.closest('.calculatorBloks').querySelector('.resultsIutput').querySelector('#'+idCal).querySelector('.itemResultValue');
            ...
            if(sum=='0'){
                ...
                perentRes.innerText = '0.00'
            }else{
                perentValue.innerText = sum.toFixed(2);
                perentRes.innerText = sum.toFixed(2)
            }
        }
    }
    CALCULATORFUNC();
...
function getValue(){
    let sum = 0;
    let allValues = totalResultValue.closest('.contentResult').querySelectorAll('.itemResultValue');
    ...
    totalResultValue.innerText = 'UAH ' + sum.toFixed(2);
}
setInterval(getValue, 250);